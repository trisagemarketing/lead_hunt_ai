...
def search_businesses(city: str, business_type: str, max_results: int = 10) -> dict:
    ...
    if not local_results:
        logger.info("0 results returned from SerpAPI.")
        return {
            "city": city,
            "business_type": business_type,
            "discovered": 0,
    ...
    # We should respect max_results for NEW businesses inserted or TOTAL businesses processed?
    # Prompt: "If max_results = 5 the module must not insert more than 5 valid new businesses from that discovery request. ... Respect the requested search result limit and avoid unnecessary API usage."
    # Since we fetch one page of local_results (usually 20), we process them and stop if we inserted `max_results` NEW leads. Or just process up to max_results from the array.
    # Actually, it says "must not insert more than 5 valid new businesses". Let's iterate and track.
    ...
    for business in local_results:
        ...
        # The prompt requires lead_id generation based on business_name, city, phone.
        # But Lead dataclass handles lead_id automatically on init.
        ...
        lead = Lead(
            ...
            # Note: actual maps url in serpapi might be in different fields, typically `place_id` or similar, we'll try to find a URL. `link` is sometimes present, or we leave it empty if not.
