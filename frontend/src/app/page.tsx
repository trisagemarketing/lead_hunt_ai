...
import { ExternalLink, CheckCircle, Mail, Send, Activity, MessageSquare, LayoutDashboard, Search, Users, Settings, Globe, Phone, Hash as Instagram, ThumbsUp as Facebook, Link as LinkIcon, Building2, Bell, ChevronDown, ChevronLeft, Menu, ChevronRight } from "lucide-react";
                ...
                <div className="animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <div className="space-y-5 max-w-md bg-slate-50 p-6 rounded-lg border border-slate-100">
                    ...
                    <div className="pt-2">
                      ...
                      >
                        {startingEngine ? <Activity className="w-5 h-5 mr-2 animate-spin" /> : <Search className="w-5 h-5 mr-2" />}
                ...
                <div className="animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="border border-slate-200 p-6 rounded-lg bg-white shadow-sm hover:border-[#654CA5]/30 transition-colors">
                      ...
                      <div className="text-4xl font-bold text-slate-800">{leads.length}</div>
                    </div>
                    <div className="border border-red-100 p-6 rounded-lg bg-red-50/50 shadow-sm relative overflow-hidden">
                      ...
                      <div className="text-4xl font-bold text-red-600">{leads.filter(l => l.lead_tier === 'HOT').length}</div>
                    ...
                    <div className="border border-green-100 p-6 rounded-lg bg-green-50/50 shadow-sm relative overflow-hidden">
                      ...
                      <div className="text-4xl font-bold text-green-700">{leads.filter(l => l.email_message).length}</div>
                      <div className="absolute top-0 right-0 p-4 opacity-10">
                ...
                <div className="animate-in fade-in zoom-in-95 duration-300">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                    <div className="flex items-center gap-3">
                       <h2 className="text-xl font-bold text-slate-800 mr-2">Pipeline ({filteredLeads.length})</h2>
                       ...
                       <button onClick={fetchLeads} className="border border-slate-300 bg-white hover:bg-slate-50 text-slate-700 px-4 py-2 rounded text-sm shadow-sm transition-colors font-medium flex items-center active:scale-95">
                          <Activity className={`w-3.5 h-3.5 mr-2 ${loading ? 'animate-spin' : ''}`} /> Refresh
                       </button>
                    </div>
